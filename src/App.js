...
export default function App() {
  ...
  const [highlightModal, setHighlightModal] = useState(null); // { dateKey }
  ...
  // 起動時：日付をまたいでいたら自動でログ保存して終了 / 一時停止中に落ちていた場合の復旧
  useEffect(() => {
    const now = Date.now();
    const todayStart = new Date(); todayStart.setHours(0,0,0,0);

    // 一時停止中にアプリが終了していた場合 → 30秒経過済みとしてログを確定する
    if (pausedSubject && pausedTimerStart && pausedAt) {
      const pauseEnd = pausedAt; // 一時停止を開始した瞬間が、勉強終了＝休憩開始の時刻
      setTimerLogs(prev => {
        if (pauseEnd - pausedTimerStart < 5000) return prev;
        return [...prev, { subjectId: pausedSubject, start: pausedTimerStart, end: pauseEnd }];
      });
      // 休憩は一時停止開始時刻からこの再起動時刻まで（30秒超えているはずなのでそのまま記録）
      setBreakLogs(prev => {
        if (now - pauseEnd < 5000) return prev;
        return [...prev, { start: pauseEnd, end: now }];
      });
  ...
  return (
    <div style={{ width:"100%", background:"#0D0F16", height:"100vh", display:"flex", flexDirection:"column", fontFamily:"'M PLUS Rounded 1c','Noto Sans JP','Hiragino Sans',sans-serif", overflow:"hidden" }}>
      ...
      {splashVisible && (
        <div onClick={closeSplash} style={{ position:"fixed", inset:0, zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", padding:32, background:"#0D0F16", transition:"opacity 0.4s", opacity: splashFading?0:1, cursor:"pointer" }}>
          <div style={{ textAlign:"center", maxWidth:320 }}>
            ...
            <div style={{ fontSize:18, fontWeight:800, color:"#F0EDE6", lineHeight:1.5, marginBottom:8, letterSpacing:-0.3 }}>
        ...
        {tab === "calendar" && (
          <div style={{ display:"flex", flexDirection:"column", paddingBottom:65 }}>
            ...
            <div style={{ display:"flex", background:"#13151F", borderRadius:12, padding:4, margin:"12px 18px 4px", border:"1px solid #1C1F2E" }}>
              <button onClick={() => setCalendarInnerTab("time")} style={{ flex:1, padding:"9px", borderRadius:9, border:"none", background: calendarInnerTab==="time"?"#1E3A5F":"transparent", color: calendarInnerTab==="time"?"#60A5FA":"#4B5563", fontSize:12, fontWeight:700, cursor:"pointer", transition:"all 0.15s" }}>⏱ 時間</button>
              <button onClick={() => setCalendarInnerTab("task")} style={{ flex:1, padding:"9px", borderRadius:9, border:"none", background: calendarInnerTab==="task"?"#1E3A5F":"transparent", color: calendarInnerTab==="task"?"#60A5FA":"#4B5563", fontSize:12, fontWeight:700, cursor:"pointer", transition:"all 0.15s" }}>📋 タスク</button>
              <button onClick={() => setCalendarInnerTab("highlight")} style={{ flex:1, padding:"9px", borderRadius:9, border:"none", background: calendarInnerTab==="highlight"?"#2D1B4E":"transparent", color: calendarInnerTab==="highlight"?"#C084FC":"#4B5563", fontSize:12, fontWeight:700, cursor:"pointer", transition:"all 0.15s" }}>⭐ 強調</button>
            </div>
              ...
              {Array.from({ length: daysInMonth }, (_,i) => {
                ...
                return (
                  <div key={day} onClick={() => calendarInnerTab === "highlight" ? setHighlightModal({ dateKey: key }) : openModal(key)}
                    style={{ minHeight:72, padding:"6px 4px 4px", background: isToday?"#1A2235":"transparent", borderTop:"1px solid #1C1F2E", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:2, WebkitTapHighlightColor:"transparent", position:"relative" }}>
                    {/* 日付 */}
            ...
            <div style={{ display:"flex", gap:12, padding:"12px 18px 4px", flexWrap:"wrap" }}>
              {calendarInnerTab === "time" ? <>
                {subjects.filter(s => {
                  ...
                  return Array.from({length: new Date(y,m+1,0).getDate()}, (_,i) => toKey(new Date(y,m,i+1))).some(k => getSecsForDate(k, s.id) > 0);
                }).map(s => (
                  <div key={s.id} style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:"#4B5563" }}>
                    <div style={{ width:10, height:4, borderRadius:2, background:s.color }} /> {s.short}
                  </div>
                ))}
              </> : calendarInnerTab === "task" ? <>
                          ...
                          <Draggable key={sub.id} draggableId={sub.id} index={idx}>
                            {(provided, snapshot) => (
                              ...
                              >
                                ...
                                <div style={{ fontSize:18, fontWeight:700, color: isRunning?getSub(sub.id).color:secs>0?"#9CA3AF":"#2A3040", fontVariantNumeric:"tabular-nums", marginRight:8 }}>
      ...
      {highlightModal && (() => {
        ...
        const HIGHLIGHT_COLORS = COLOR_PALETTE;
        const editingH = highlightModal.editingId ? highlights.find(h => h.id === highlightModal.editingId) : null;
        ...
        return (
          <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", zIndex:50, display:"flex", alignItems:"flex-end", backdropFilter:"blur(6px)" }} onClick={() => { setHighlightModal(null); setHighlightForm({ text:"", color:"#F87171" }); }}>
            <div style={{ width:"100%", maxWidth:430, margin:"0 auto", background:"#13151F", borderRadius:"20px 20px 0 0", padding:"20px 20px env(safe-area-inset-bottom,24px)", maxHeight:"88vh", overflowY:"auto" }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
                <div>
                  <div style={{ fontSize:12, color:"#4B5563", marginBottom:4 }}>{dateLabel}</div>
                  <div style={{ fontSize:18, fontWeight:800, color:"#C084FC" }}>
                    {isEditing ? "✏️ 強調事項を編集" : "⭐ 強調事項"}
                  </div>
                </div>
                {isEditing && (
                  <button onClick={() => setHighlightModal({ dateKey: dk })} style={{ background:"#1C1F2E", border:"none", color:"#9CA3AF", borderRadius:10, padding:"8px 14px", fontSize:13, cursor:"pointer", fontWeight:600 }}>← 戻る</button>
                )}
              </div>
              ...
              {!isEditing && dayHighlights.length > 0 && (
                <div style={{ marginBottom:20 }}>
                  {dayHighlights.map(h => (
                    <div key={h.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 14px", background:"#0D0F16", borderRadius:10, marginBottom:8, borderLeft:"4px solid "+h.color, cursor:"pointer" }}
                      onClick={() => { setHighlightModal({ dateKey: dk, editingId: h.id }); setHighlightForm({ text: h.text, color: h.color }); }}>
                      <span style={{ fontSize:14, fontWeight:700, color:h.color }}>{h.text}</span>
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        <span style={{ fontSize:11, color:"#4B5563" }}>タップで編集</span>
                        <button onClick={e => { e.stopPropagation(); deleteHighlight(h.id); }} style={{ background:"none", border:"none", color:"#374151", fontSize:18, cursor:"pointer", padding:"2px 6px" }}>×</button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <input
                ...
                onChange={e => setHighlightForm(f => ({ ...f, text: e.target.value }))}
                placeholder="例：答練4、財務締切、模試…"
                style={{ width:"100%", boxSizing:"border-box", padding:"13px 14px", borderRadius:12, background:"#0D0F16", border:"1.5px solid "+(highlightForm.text?"#C084FC":"#1C1F2E"), color:"#E5E1D8", fontSize:14, outline:"none", fontFamily:"inherit", marginBottom:14 }}
              />
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(8,1fr)", gap:10, marginBottom:18 }}>
                {HIGHLIGHT_COLORS.map(c => (
                  <button key={c} onClick={() => setHighlightForm(f => ({ ...f, color: c }))} style={{ width:"100%", aspectRatio:"1", borderRadius:10, background:c, border: highlightForm.color===c?"3px solid #fff":"3px solid transparent", cursor:"pointer" }} />
                ))}
              </div>
              {/* プレビュー */}
              {highlightForm.text && (
                <div style={{ padding:"10px 14px", background:"#0D0F16", borderRadius:10, marginBottom:14, borderLeft:"4px solid "+highlightForm.color }}>
                  <span style={{ fontSize:14, fontWeight:700, color:highlightForm.color }}>{highlightForm.text}</span>
                </div>
              )}
              ...
              {isEditing ? (
                <div style={{ display:"flex", gap:10 }}>
                  ...
                  <button onClick={() => {
                    if (!highlightForm.text.trim()) return;
                    setHighlights(prev => prev.map(h => h.id === highlightModal.editingId ? { ...h, text: highlightForm.text.trim(), color: highlightForm.color } : h));
                    setHighlightModal({ dateKey: dk });
                    setHighlightForm({ text:"", color:"#F87171" });
                  }} disabled={!highlightForm.text.trim()} style={{ flex:2, padding:"14px", borderRadius:14, border:"none", background: highlightForm.text.trim()?"linear-gradient(135deg,#9333EA,#C084FC)":"#1C1F2E", color: highlightForm.text.trim()?"#fff":"#374151", fontSize:15, fontWeight:800, cursor: highlightForm.text.trim()?"pointer":"not-allowed" }}>保存する</button>
                </div>
              ) : (
                <button onClick={() => {
                  addHighlight(dk, highlightForm.text, highlightForm.color);
                  setHighlightForm({ text:"", color:"#F87171" });
                }} disabled={!highlightForm.text.trim()} style={{ width:"100%", padding:"16px", borderRadius:14, border:"none", background: highlightForm.text.trim()?"linear-gradient(135deg,#9333EA,#C084FC)":"#1C1F2E", color: highlightForm.text.trim()?"#fff":"#374151", fontSize:15, fontWeight:800, cursor: highlightForm.text.trim()?"pointer":"not-allowed" }}>追加する</button>
              )}
            </div>
          </div>
        );
...
function SessionCard({ s, subjects, showDate, onTap, onToggleDone }) {
  ...
  const d = new Date(s.date+"T00:00:00");
  return (
    <div style={{ padding:"14px 16px", background:"#13151F", borderRadius:14, borderLeft:"4px solid "+(s.done?sub.color+"50":sub.color), marginBottom:10, WebkitTapHighlightColor:"transparent", opacity: s.done?0.75:1 }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        {onToggleDone && (
          <button onClick={() => onToggleDone(s.id)} style={{ flexShrink:0, marginLeft:12, width:32, height:32, borderRadius:"50%", border:"1.5px solid "+(s.done?"#16A34A":"#374151"), background: s.done?"#16A34A":"transparent", color: s.done?"#fff":"#374151", fontSize:14, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", WebkitTapHighlightColor:"transparent" }}>
            {s.done ? "✓" : ""}
          </button>
        )}
      </div>
