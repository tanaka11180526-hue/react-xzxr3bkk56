        ...
        {tab === "today" && (() => {
          ...
          return (
            <div style={{ padding:"16px 18px" }}>
              ...
              {upcoming.length > 0 && todayDoneTab === "all" && (
                <CollapsibleReviewSection
                  title="今後7日間の復習"
                  reviews={upcoming}
                  subjects={subjects}
                    ...
                    return (
                      <div key={i} style={{ padding:"12px 14px", borderTop: i>0?"1px solid #1C1F2E":"none", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                        <div onClick={() => onTap(r)} style={{ display:"flex", alignItems:"center", gap:8, flex:1, cursor:"pointer", WebkitTapHighlightColor:"transparent" }}>
                          ...
                          <span style={{ fontSize:13, color: done?"#4B5563":"#9CA3AF", fontWeight:600, textDecoration: done?"line-through":"none" }}>{r.intervalLabel}</span>
                          {showDate && r.dateLabel && <span style={{ fontSize:12, color:"#6B7280" }}>{r.dateLabel}</span>}
                        </div>
                        {onToggleReviewDone && (
