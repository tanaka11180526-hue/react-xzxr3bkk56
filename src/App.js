...
export default function App() {
  ...
  useEffect(() => { if (activeSubject) localStorage.setItem("cpa_active_subject", activeSubject); else localStorage.removeItem("cpa_active_subject"); }, [activeSubject]);
  useEffect(() => { if (timerStart) localStorage.setItem("cpa_timer_start", String(timerStart)); else localStorage.removeItem("cpa_timer_start"); }, [timerStart]);
  useEffect(() => { if (breakStart) localStorage.setItem("cpa_break_start", String(breakStart)); else localStorage.removeItem("cpa_break_start"); }, [breakStart]);
  ...
  function getLogsForDate(dateKey) {
    ...
    const logs = timerLogs.filter(l => l.start >= start && l.start < end);
    if (dateKey === toKey(new Date()) && activeSubject && timerStart) {
      logs.push({ subjectId: activeSubject, start: timerStart, end: Date.now(), live: true });
    }
    return logs;
  }
  function getBreakLogsForDate(dateKey) {
    const start = new Date(dateKey+"T00:00:00").getTime();
    const end = start + 86400000;
    const logs = breakLogs.filter(l => l.start >= start && l.start < end);
    if (dateKey === toKey(new Date()) && breakStart) {
      logs.push({ start: breakStart, end: Date.now(), live: true });
    }
    return logs;
  }
  ...
  return (
    <div style={{ width:"100%", background:"#0D0F16", height:"100vh", display:"flex", flexDirection:"column", fontFamily:"'Noto Sans JP','Hiragino Sans',sans-serif", overflow:"hidden" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column" }}>
                ...
                <div style={{ textAlign:"center", marginBottom:8 }}>
                  ...
                  {isToday && activeSubject && (
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:5, marginTop:5 }}>
                      ...
                      <span style={{ fontSize:12, color: getSub(activeSubject).color, fontWeight:700 }}>{getSub(activeSubject).label} 計測中</span>
                    </div>
                  )}
                  {isToday && !activeSubject && breakStart && (
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6, marginTop:5 }}>
                      ...
                      <span style={{ fontSize:12, color:"#FBBF24", fontWeight:700 }}>
                        休憩中 {fmtTime(Math.floor((Date.now() - breakStart) / 1000))}
                      </span>
              ...
              <div>
                {subjects.map((sub, idx) => {
                  ...
                  return (
                    <div key={sub.id} style={{ display:"flex", alignItems:"center", padding:"13px 18px", borderBottom:"1px solid #1C1F2E", background: isRunning ? colorBg(sub.color) : "transparent" }}>
                      {/* 並び替えボタン */}
                      <div style={{ display:"flex", flexDirection:"column", gap:2, marginRight:10, flexShrink:0 }}>
                        <button onClick={() => { if(idx===0) return; const a=[...subjects]; [a[idx-1],a[idx]]=[a[idx],a[idx-1]]; setSubjects(a); }} style={{ background:"none", border:"none", color: idx===0?"#1C1F2E":"#374151", fontSize:12, cursor: idx===0?"default":"pointer", padding:"2px 4px", lineHeight:1 }}>▲</button>
                        <button onClick={() => { if(idx===subjects.length-1) return; const a=[...subjects]; [a[idx+1],a[idx]]=[a[idx],a[idx+1]]; setSubjects(a); }} style={{ background:"none", border:"none", color: idx===subjects.length-1?"#1C1F2E":"#374151", fontSize:12, cursor: idx===subjects.length-1?"default":"pointer", padding:"2px 4px", lineHeight:1 }}>▼</button>
                      </div>
                      {isToday ? (
                      ...
                      <div style={{ fontSize:18, fontWeight:700, color: isRunning?"#F0EDE6":secs>0?"#9CA3AF":"#2A3040", fontVariantNumeric:"tabular-nums", marginRight: isToday?8:0 }}>
                        {fmtTime(secs)}
                      </div>
                      {isToday && (
                        <div style={{ display:"flex", gap:4 }}>
                          ...
                          {secs > 0 && <button onClick={() => resetTimer(sub.id)} style={{ background:"none", border:"none", color:"#2A3040", fontSize:18, cursor:"pointer", padding:"4px" }}>⋮</button>}
                        </div>
                      )}
                    </div>
                  );
                      ...
                      <div key={hour} style={{ display:"flex", alignItems:"center", marginBottom:3 }}>
                        ...
                        <div style={{ display:"flex", gap:2, flex:1 }}>
                          {rowBlocks.map(({ color, log, isBreak }, i) => (
                            <div key={i} onClick={() => { if (log && !log.live) setLogDetailModal({ log }); }} style={{ flex:1, height:24, borderRadius:4, background: isBreak?"#374151":color||(isCurrentHour?"#1A2035":hasAny?"#1C1F2E":"#13151F"), border: (color||isBreak)?"none":"1px solid "+(isCurrentHour?"#1E3050":"#1A1D27"), cursor: color&&!log?.live?"pointer":"default", opacity: isBreak?0.6:1 }} />
                          ))}
                        </div>
                      </div>
                    );
      ...
      {logDetailModal && (() => {
        ...
        const sub = getSub(log.subjectId);
        const secs = Math.floor((log.end-log.start)/1000);
        const startD = new Date(log.start), endD = new Date(log.end);
        const fmt = d => String(d.getHours()).padStart(2,"0")+":"+String(d.getMinutes()).padStart(2,"0");
        return (
          <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.8)", zIndex:70, display:"flex", alignItems:"center", justifyContent:"center", padding:"24px", backdropFilter:"blur(6px)" }} onClick={() => setLogDetailModal(null)}>
            <div style={{ width:"100%", maxWidth:340, background:"#13151F", borderRadius:20, padding:"24px", border:"1px solid "+sub.color+"40" }} onClick={e => e.stopPropagation()}>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16 }}>
                <div style={{ width:12, height:12, borderRadius:3, background:sub.color }} />
                <div style={{ fontSize:16, fontWeight:800, color:sub.color }}>{sub.label}</div>
                {log.manual && <span style={{ fontSize:10, padding:"2px 7px", background:"#1C1F2E", borderRadius:5, color:"#4B5563", fontWeight:600 }}>手動追加</span>}
              </div>
              ...
              <div style={{ display:"flex", gap:10 }}>
                <button onClick={() => setLogDetailModal(null)} style={{ flex:1, padding:"13px", borderRadius:12, border:"1.5px solid #2A3040", background:"transparent", color:"#9CA3AF", fontSize:13, fontWeight:700, cursor:"pointer" }}>閉じる</button>
                <button onClick={() => deleteLog(log)} style={{ flex:1, padding:"13px", borderRadius:12, border:"none", background:"#7F1D1D", color:"#F87171", fontSize:13, fontWeight:800, cursor:"pointer" }}>この記録を削除</button>
              </div>
            </div>
          </div>
        );
