      ...
      <div style={{ position:"fixed", bottom:0, left:0, width:"100%", background:"#0D0F16", borderTop:"1px solid #1C1F2E", display:"flex", padding:"8px 0 env(safe-area-inset-bottom,8px)", zIndex:20 }}>
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)} style={{ flex:1, border:"none", background:"none", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:3, padding:"6px 0", WebkitTapHighlightColor:"transparent" }}>
            <div style={{ fontSize: t.id==="ai" ? 24 : 28, fontWeight: t.id==="ai" ? 800 : 400, color: tab===t.id && t.id==="ai" ? "#A78BFA" : "inherit", lineHeight:1 }}>{t.icon}</div>
            <div style={{ fontSize:12, fontWeight:700, color: tab===t.id ? (t.id==="ai"?"#A78BFA":"#60A5FA") : "#374151" }}>{t.label}</div>
...
// 2段階折りたたみ復習セクション
function CollapsibleReviewSection({ title, reviews, subjects, onTap, showDate }) {
  ...
  }, [open, expandedIds, groups.length]);
            ...
            return (
              <div key={g.key} style={{ marginBottom:8 }}>
                <button onClick={() => toggleExpand(g.key)} style={{ width:"100%", background:"none", border:"none", cursor:"pointer", padding:0, WebkitTapHighlightColor:"transparent" }}>
                  <div style={{ padding:"12px 14px", background:"#13151F", borderRadius: isExpanded?"12px 12px 0 0":12, borderLeft:"4px solid "+sub.color, outline: isExpanded?"1.5px solid "+sub.color+"60":"1.5px solid transparent", display:"flex", alignItems:"center", justifyContent:"space-between", transition:"border-radius 0.25s ease" }}>
                    <div style={{ textAlign:"left", flex:1, minWidth:0 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:4 }}>
                        <span style={{ fontSize:12, color:sub.color, fontWeight:800 }}>🔄 {sub.label}</span>
                      ...
                      <div style={{ fontSize:13, color:"#6B7280", lineHeight:1.4, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{g.content.length>45?g.content.slice(0,45)+"…":g.content}</div>
                    </div>
                ...
                <div style={{
                  overflow:"hidden",
                  maxHeight: isExpanded ? g.items.length * 60 : 0,
                  ...
                  {g.items.map((r, i) => (
                    <div key={i} onClick={() => onTap(r)} style={{ padding:"10px 14px", borderTop: i>0?"1px solid #1C1F2E":"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"space-between", WebkitTapHighlightColor:"transparent" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        <div style={{ width:6, height:6, borderRadius:"50%", background:sub.color, flexShrink:0 }} />
                        <span style={{ fontSize:12, color:"#9CA3AF", fontWeight:600 }}>{r.intervalLabel}</span>
                        {showDate && r.dateLabel && <span style={{ fontSize:11, color:"#374151" }}>{r.dateLabel}</span>}
                      </div>
                      <span style={{ fontSize:11, color:"#374151" }}>›</span>
                    </div>
                  ))}
