          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            {!open && (
              <div style={{ display:"flex", position:"relative", height:28 }}>
                {stackColors.map((c, i) => (
                  <div key={i} style={{ position:"absolute", left: i*10, width:44, height:28, borderRadius:8, background:"#13151F", borderLeft:"4px solid "+c, border:"1px solid #1C1F2E", zIndex: stackColors.length - i }} />
                ))}
                {groups.length > 3 && (
                  <div style={{ position:"absolute", left: 3*10+4, fontSize:10, color:"#4B5563", top:"50%", transform:"translateY(-50%)", fontWeight:700 }}>+{groups.length-3}</div>
                )}
              </div>
            )}
            <span style={{ fontSize:16, color:"#4B5563" }}>{open ? "▲" : "▼"}</span>
          </div>
        </div>
      </button>
        ...
        return (
          <div key={g.key} style={{ marginBottom:8 }}>
            ...
            <button onClick={() => toggleExpand(g.key)} style={{ width:"100%", background:"none", border:"none", cursor:"pointer", padding:0, WebkitTapHighlightColor:"transparent" }}>
              <div style={{ padding:"12px 14px", background:"#13151F", borderRadius: isExpanded?"12px 12px 0 0":12, borderLeft:"4px solid "+sub.color, border:"1.5px solid "+(isExpanded?sub.color+"60":"transparent"), borderLeft:"4px solid "+sub.color, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                ...
                <span style={{ fontSize:14, color:"#374151", marginLeft:10 }}>{isExpanded?"▲":"▼"}</span>
              </div>
            </button>

            {/* 展開後：個別復習カード */}
            {isExpanded && (
              <div style={{ background:"#0D0F16", borderRadius:"0 0 12px 12px", border:"1.5px solid "+sub.color+"60", borderTop:"none", overflow:"hidden" }}>
                {g.items.map((r, i) => (
