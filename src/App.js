...
// ── LEVEL定義 ──
const CHARA_LEVELS = [
  { level:1,  name:"タマゴ",     minH:0,   color:"#9CA3AF", evolveH:1   },
  { level:2,  name:"ホシコ",     minH:1,   color:"#60A5FA", evolveH:3   },
  { level:3,  name:"ホシコ★",   minH:3,   color:"#818CF8", evolveH:6   },
  { level:4,  name:"フクミン",   minH:6,   color:"#A78BFA", evolveH:12  },
  { level:5,  name:"フクミン★", minH:12,  color:"#C084FC", evolveH:20  },
  { level:6,  name:"フクロード", minH:20,  color:"#E879F9", evolveH:35  },
  { level:7,  name:"フクロード★",minH:35, color:"#F472B6", evolveH:50  },
  { level:8,  name:"マスター",   minH:50,  color:"#FBBF24", evolveH:75  },
  { level:9,  name:"グランド",   minH:75,  color:"#F59E0B", evolveH:100 },
  { level:10, name:"伝説のCPA",  minH:100, color:"#EF4444", evolveH:999 },
];
...
// ── キャラSVG ──
function CharaSVG({ level, size = 140, animated = true }) {
  const lv = level;
  const color = CHARA_LEVELS.find(l => l.level === lv)?.color || "#9CA3AF";

  // タマゴ（Lv1）
  if (lv === 1) return (
    <svg width={size} height={size} viewBox="0 0 100 100">
      <ellipse cx="50" cy="54" rx="28" ry="34" fill="#E5E7EB" stroke="#D1D5DB" strokeWidth="2"/>
      <ellipse cx="50" cy="54" rx="28" ry="34" fill="none" stroke="#9CA3AF" strokeWidth="1" strokeDasharray="4 3" opacity="0.5"/>
      <ellipse cx="42" cy="46" rx="4" ry="5" fill="white" opacity="0.6"/>
    </svg>
  );

  // フクロウ系キャラ（Lv2〜）
  const bodyColor = color;
  const bellyColor = lv >= 8 ? "#FDE68A" : lv >= 5 ? "#E0E7FF" : "#F0EDE6";
  const eyeGlow = lv >= 6;
  const hasCrown = lv === 10;
  const hasAura = lv >= 8;
  const wings = lv >= 4;
  const stars = lv >= 7;
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" style={{ overflow:"visible" }}>
      ...
      {hasAura && (
        <>
          <circle cx="50" cy="55" r="42" fill={color} opacity="0.08"/>
          <circle cx="50" cy="55" r="36" fill={color} opacity="0.07"/>
        </>
      )}
      ...
      {wings && (
        <>
          <ellipse cx="22" cy="62" rx="12" ry="8" fill={bodyColor} opacity="0.8" transform="rotate(-20 22 62)"/>
          <ellipse cx="78" cy="62" rx="12" ry="8" fill={bodyColor} opacity="0.8" transform="rotate(20 78 62)"/>
        </>
      )}

      {/* 体 */}
      <ellipse cx="50" cy="62" rx="22" ry="26" fill={bodyColor}/>
      {/* お腹 */}
      <ellipse cx="50" cy="66" rx="13" ry="16" fill={bellyColor} opacity="0.9"/>

      {/* 頭 */}
      <circle cx="50" cy="38" r="20" fill={bodyColor}/>

      {/* 耳羽（ミミズク風） */}
      <polygon points="34,22 30,12 38,18" fill={bodyColor}/>
      <polygon points="66,22 70,12 62,18" fill={bodyColor}/>
      <polygon points="34,22 31,15 37,19" fill={bellyColor} opacity="0.7"/>
      <polygon points="66,22 69,15 63,19" fill={bellyColor} opacity="0.7"/>

      {/* 目（白目） */}
      <circle cx="42" cy="38" r="7" fill="white"/>
      <circle cx="58" cy="38" r="7" fill="white"/>

      {/* 目（瞳） */}
      <circle cx="43" cy="39" r="4.5" fill={lv >= 6 ? color : "#1e293b"}/>
      <circle cx="59" cy="39" r="4.5" fill={lv >= 6 ? color : "#1e293b"}/>
      {/* 光 */}
      <circle cx="44.5" cy="37.5" r="1.5" fill="white"/>
      <circle cx="60.5" cy="37.5" r="1.5" fill="white"/>
      {eyeGlow && (
        <>
          <circle cx="43" cy="39" r="5.5" fill="none" stroke={color} strokeWidth="1.5" opacity="0.6"/>
          <circle cx="59" cy="39" r="5.5" fill="none" stroke={color} strokeWidth="1.5" opacity="0.6"/>
        </>
      )}

      {/* くちばし */}
      <polygon points="50,44 46,48 54,48" fill="#FBBF24"/>

      {/* 足 */}
      <line x1="44" y1="86" x2="40" y2="93" stroke="#FBBF24" strokeWidth="2.5" strokeLinecap="round"/>
      <line x1="40" y1="93" x2="36" y2="93" stroke="#FBBF24" strokeWidth="2" strokeLinecap="round"/>
      <line x1="40" y1="93" x2="40" y2="97" stroke="#FBBF24" strokeWidth="2" strokeLinecap="round"/>
      <line x1="40" y1="93" x2="44" y2="96" stroke="#FBBF24" strokeWidth="2" strokeLinecap="round"/>
      <line x1="56" y1="86" x2="60" y2="93" stroke="#FBBF24" strokeWidth="2.5" strokeLinecap="round"/>
      <line x1="60" y1="93" x2="64" y2="93" stroke="#FBBF24" strokeWidth="2" strokeLinecap="round"/>
      <line x1="60" y1="93" x2="60" y2="97" stroke="#FBBF24" strokeWidth="2" strokeLinecap="round"/>
      <line x1="60" y1="93" x2="56" y2="96" stroke="#FBBF24" strokeWidth="2" strokeLinecap="round"/>

      {/* 星（高レベル） */}
      {stars && (
        <>
          <text x="18" y="30" fontSize="10" fill={color} opacity="0.9">★</text>
          <text x="72" y="30" fontSize="10" fill={color} opacity="0.9">★</text>
        </>
      )}
      ...
      {hasCrown && (
        <>
          <polygon points="50,8 44,16 50,13 56,16" fill="#FBBF24" stroke="#F59E0B" strokeWidth="1"/>
          <circle cx="50" cy="9" r="2" fill="#EF4444"/>
          <circle cx="44" cy="16" r="1.5" fill="#3B82F6"/>
          <circle cx="56" cy="16" r="1.5" fill="#3B82F6"/>
        </>
      )}
    </svg>
  );
            ...
            return (
              <div key={lv.level} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 10px", borderRadius:10, background: isCurrent ? lv.color + "20" : "transparent", border: isCurrent ? `1.5px solid ${lv.color}` : "1.5px solid transparent" }}>
                <div style={{ width:28, height:28, flexShrink:0 }}>
                  <CharaSVG level={lv.level} size={28} animated={false} />
                </div>
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:11, color: unlocked ? "#6B7280" : "#2A3040" }}>{lv.minH}h〜</div>
                </div>
                <div style={{ fontSize:16 }}>{unlocked ? "✓" : "🔒"}</div>
              </div>
            );
          })}
        </div>
