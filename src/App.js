                  ...
                  <div key={day} onClick={() => calendarInnerTab === "highlight" ? setHighlightModal({ dateKey: key }) : openModal(key)}
                    ...
                    {calendarInnerTab === "task" && (
                      <>
                        ...
                        {reviewDots.length > 0 && (
                          <div style={{ display:"flex", flexWrap:"wrap", gap:2, justifyContent:"center" }}>
                            {reviewDots.map((r,ri) => <div key={"r"+ri} style={{ width:6, height:6, borderRadius:"50%", background:"transparent", border:"1.5px solid "+getSub(r.subject).color }} />)}
                          </div>
                        )}
                      </>
                    )}
        ...
        return (
          <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", zIndex:50, display:"flex", alignItems:"flex-end", backdropFilter:"blur(6px)" }} onClick={() => { setHighlightModal(null); setHighlightForm({ text:"", color:"#F87171" }); }}>
            <div style={{ width:"100%", maxWidth:430, margin:"0 auto", background:"#13151F", borderRadius:"20px 20px 0 0", padding:"20px 20px env(safe-area-inset-bottom,24px)", maxHeight:"88vh", overflowY:"auto" }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ fontSize:18, fontWeight:800, color:"#C084FC", marginBottom:20 }}>⭐ 強調事項</div>

              {/* 既存リスト */}
              {dayHighlights.length > 0 && (
                <div style={{ marginBottom:20 }}>
                  {dayHighlights.map(h => (
                    <div key={h.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 14px", background:"#0D0F16", borderRadius:10, marginBottom:8, borderLeft:"4px solid "+h.color }}>
                      <span style={{ fontSize:14, fontWeight:700, color:h.color }}>{h.text}</span>
                      <button onClick={() => deleteHighlight(h.id)} style={{ background:"none", border:"none", color:"#374151", fontSize:18, cursor:"pointer", padding:"2px 6px" }}>×</button>
                    </div>
                  ))}
                </div>
              )}
